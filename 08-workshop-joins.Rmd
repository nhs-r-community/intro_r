...
</br> From this new table we can derive a rate.
...
For every value of Afghanistan in tb_cases, there are 4 in tb_pop
...
```{r}
tb_cases %>% 
  left_join(new_table, by = c("country" == "Place",
                              "year" = "Yr"))
```

new_table is an imaginary table 

new_table has columns named Place and Yr that match the tb_cases columns country and year

---

# Other joins

Like SQL, dplyr has left_join, inner_join and full_join. Joins that are slightly different to SQL are:
